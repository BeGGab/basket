...
The experiment ships as a stack of four dependent PRs, one per layer: domain + emulator (TZ-001), assistants (TZ-004), runtime + scenario engine (TZ-002), `/sim` viewer (TZ-003). TZ-BASKET-005 is a later domain iteration on expiration / silence / time (SPEC v0.3). TZ-BASKET-006 closes SPEC OQ-001 (price semantics, SPEC v0.4). TZ-BASKET-007 splits OQ-002 into OQ-002A/OQ-002B (SPEC v0.5) without introducing Package or VolumePrice entities. TZ-BASKET-008 keeps both OPEN (conclusion B, SPEC v0.6): package-unit deals need no stored contents; standing announcements are not Offers. TZ-BASKET-009 records catalog/spec reconstruction only (no business-flow observation, SPEC remains v0.6). TZ-BASKET-010 records Stage-1 source search (SOURCE ABSENT in inspected files). TZ-BASKET-011 exercises the Stage-1 buyer/seller path and tests seller-config **executability**: OQ-002A/B are **INCONCLUSIVE** (seller-configured-constraint observation **NOT OBTAINED**). TZ-BASKET-012 attempts seller-facing configuration (`FLOW-012-*`): Impl **NOT EXECUTABLE** on inspected surfaces; Package and PriceSchedule are still not introduced. SPEC remains v0.6. OQ-003 may proceed independently but does not replace a seller-facing configuration flow. Each layer keeps its own runner: domain scenarios in `tests/run.ts`, engine in `tests/runtime.ts`, assistants in `tests/assistants.ts`. `/sim` is a viewer, not acceptance.
