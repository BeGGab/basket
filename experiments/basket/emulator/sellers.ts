  ...
  return {
    ...
    tick(world, sellerPurchaseId) {
      ...
      if (profile === "TimeDiscountSeller") {
        ...
        // Only a seller-side proposal may be discounted; a buyer Offer is answered, not rewritten.
      ...
      if (profile === "PartialAvailabilitySeller") {
        const active = sp.activeOfferId ? world.offerById(sp.activeOfferId) : null;
        if (!active) return;
        // Stock adjustment applies to our own proposal; a buyer Offer goes through respondToBuyerOffer.
        if (active.actor === "BUYER") return;
