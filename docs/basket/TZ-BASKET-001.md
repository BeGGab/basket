...
Lifecycle односторонний: `PROPOSED → ACCEPTED | REJECTED` (I-032). Решённую Substitution нельзя переиграть — новое намерение оформляется новой Substitution.
...
Сценарий `requested=20, agreed=20, actual=5` не делает STABLE недействительным. `fulfilledQuantity` не добавляется в центральную модель — исполнение моделируется внешним mock-сценарием. `mockFulfill()` пишет фактически доставленное количество и проверяет FULFILLMENT-checkpoint именно по нему; `partialFulfillmentAllowed = false` — работающая policy: недопоставка отклоняется, а не записывается.
...
Детерминированный запуск: контролируемое время, фиксированные решения, цены, availability. Не вводить автоматически `SELLER_UNRESPONSIVE`. Stock race не решать скрытой allocation. Claim для stock-conflict = quantity **active** Offer, не agreed. `PartialAvailabilitySeller` смотрит только catalog stock; competing claims / allocation вне эксперимента. На `tick()` профили правят только собственное предложение (`SELLER`/`SYSTEM`): на buyer Offer эмулятор отвечает через `respondToBuyerOffer`, а не переписывает его.

Детерминизм обеспечен владением состоянием: каталог копируется в `setCatalog()`, меняется только через `setStock()`, а все чтения мира — замороженные проекции (I-033, I-034).
...
Expected/Actual не пишутся руками: единственный конструктор evidence — `prove()`, который сначала сверяет карту фактов с живым состоянием мира, а затем сериализует ту же карту в отчёт. Расхождение между смыслом assertion и текстом в отчёте невозможно.
