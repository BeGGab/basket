...
- **OQ-030:** Split by TZ-BASKET-007; refined by TZ-BASKET-008 (conclusion **B**, no new entity). TZ-BASKET-009 obtained **no business-flow observation** — only catalog/spec reconstruction. TZ-BASKET-010 is Stage-1 **source search**. TZ-BASKET-011 exercised Stage-1 buyer/seller deals (`FLOW-011-*`) and tested seller-config **executability** (`FLOW-011-A-CONFIG-CAPABILITY`): that row is a coverage check, not a seller-configured-constraint observation. TZ-BASKET-012 attempted the seller-facing configuration chain (`FLOW-012-*`): Impl **NOT EXECUTABLE** on inspected surfaces; seller-configured observation **NOT OBTAINED**. Observation status **INCONCLUSIVE** for both OQs. SOURCE token absence was not reused as business evidence. Canonical: **SPEC OQ-002A** OPEN. **SPEC OQ-002B** OPEN. No Package or PriceSchedule. SPEC remains v0.6.
