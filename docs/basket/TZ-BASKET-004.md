...
## Зависимость от доменного времени (SPEC v0.3)

Ассистенты используют `isOfferValid` как validity **стоящего активного предложения** (принять /
counter). Это согласовано с I-037 / I-028 / I-035 после закрытия OQ-009:

- **истёкший agreed Offer продолжает служить ценовым baseline** — соглашение — исторический факт;
...
завершённость production-корзины.
...
- [x] I-037: истёкший agreed Offer остаётся baseline; STABLE + expiry → `WAIT(TERMINAL_STATUS)`
